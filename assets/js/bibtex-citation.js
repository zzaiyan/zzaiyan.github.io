  ...
  const BibtexManager = {
    ...
    getBibtex: function (key) {
      if (
        ...
        typeof window.BibtexDatabase.getBibtex === "function"
      ) {
    ...
    getAvailableKeys: function () {
      if (
        window.BibtexDatabase &&
        typeof window.BibtexDatabase.getAvailableKeys === "function"
      ) {
  ...
  const BibtexCitation = {
    ...
    show: function (paperKey) {
      ...
      if (!BibtexManager.isDatabaseLoaded()) {
        console.error(
          "BibTeX database not loaded. Please check if bibtex data is properly inlined in scripts.html.",
        );
