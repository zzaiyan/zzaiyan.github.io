...
if git -C results diff --cached --quiet; then
  ...
  git -C results push \
    "git@github.com:zzaiyan/zzaiyan.github.io.git" HEAD:google-scholar-stats --force
...
if command -v gh >/dev/null 2>&1; then
  ...
  printf '%s\n' \
    "GitHub CLI is unavailable. Run: gh workflow run deploy.yml --repo zzaiyan/zzaiyan.github.io --ref main"
fi
